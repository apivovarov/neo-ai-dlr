...
std::vector<std::vector<float>> RunInference(DLRModelHandle model, const char* data_path, std::string input_name) {
  ...
  for (int i = 0; i < num_outputs; i++){
    if (GetDLROutput(&model, 0, outputs[i].data()) != 0) {
...
int main(int argc, char** argv) {
  ...
  for (int rep = 0; rep < 10; rep++) {
    ...
    // float argmax = -100000.0f;
    // int max_id = -95;
    // for (int i = 0; i < tvm_preds[0].size(); i++){
    //   if (tvm_preds[0][i] > argmax) {
    //     argmax = tvm_preds[0][i];
    //     max_id = i;
    //   }
    // }

    // std::cout << std::endl << "inference result: " <<max_id << std::endl;
    // for (auto v : tvm_preds) {
    //   for (auto i : v) {
    //     std::cout << i << ", ";
    //   }
